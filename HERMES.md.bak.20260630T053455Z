...
1. 先读取 `query://usage-guide`；再按当前客户端读取对应画像：Codex 读 `query://client-profile/codex`，Claude Code 读 `query://client-profile/claude-code`，其他 CLI 或不确定时读 `query://client-profile/generic-cli`。
