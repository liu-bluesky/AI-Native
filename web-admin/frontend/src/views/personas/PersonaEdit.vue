...
async function fetchDetail() {
  loading.value = true
  try {
    const { persona } = await api.get(`/personas/${route.params.id}`)
    Object.assign(form, {
      name: persona.name || '',
      tone: persona.tone || 'professional',
      verbosity: persona.verbosity || 'concise',
      language: persona.language || 'zh-CN',
      behaviors: [...(persona.behaviors || [])],
      style_hints: [...(persona.style_hints || [])],
      decision_policy: {
        risk_preference: persona.decision_policy?.risk_preference || 'balanced',
        uncertain_action: persona.decision_policy?.uncertain_action || 'ask_for_confirmation',
      },
      drift_control: {
        enabled: persona.drift_control?.enabled ?? true,
        window_days: persona.drift_control?.window_days ?? 30,
        max_drift_score: persona.drift_control?.max_drift_score ?? 0.25,
      },
    })
  } catch {
...
async function handleSave() {
  ...
  try {
    ...
    await api.put(`/personas/${route.params.id}`, payload)
    ElMessage.success('保存成功')
    router.push(`/personas/${route.params.id}`)
  } catch (e) {
