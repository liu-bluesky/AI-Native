<template>
  <div class="employee-create">
    <h3>创建 AI 员工</h3>
    <p class="page-hint">按 3 步完成配置：基础信息、能力绑定、人设与进化。</p>
    ...
    >
      ...
      <template v-else-if="currentStep === 1">
        ...
        <el-form-item label="已选规则">
          <div class="selected-line selected-rule-line">
            <el-tag
              ...
              type="success"
              class="selected-tag"
              @close="removeSelectedRule(rule.id)"
            >
              {{ rule.title }}
            </el-tag>
            <span v-if="!selectedRules.length" class="preview-empty">未选择</span>
          </div>
          <div class="field-hint">已选规则独占一行展示，避免换行挤压造成混乱。</div>
        </el-form-item>
...
async function handleCreate() {
  ...
  try {
    const payload = {
      ...form,
      rule_domains: collectRuleDomainsByRuleIds(selectedRuleIds.value),
...
onMounted(fetchSelectionOptions);
</script>
