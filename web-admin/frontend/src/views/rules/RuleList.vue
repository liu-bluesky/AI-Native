  ...
  <div v-loading="loading">
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        <el-select v-model="filterDomain" placeholder="按领域筛选" clearable size="small" class="filter-domain">
          <el-option v-for="d in domains" :key="d" :label="d" :value="d" />
        </el-select>
        <el-input v-model="keyword" placeholder="搜索关键词" size="small" class="filter-keyword"
          @keyup.enter="doSearch" clearable />
        <el-button size="small" type="primary" @click="doSearch">搜索</el-button>
        <el-button size="small" type="primary" @click="$router.push('/rules/create')">新建规则</el-button>
      </div>
    </div>
    ...
    <el-table :data="rules" stripe>
      ...
      <el-table-column label="操作" width="260" fixed="right">
        <template #default="{ row }">
          ...
          <el-button text type="primary" size="small" @click="$router.push(`/rules/${row.id}/edit`)">编辑</el-button>
    ...
    <el-dialog v-model="showMcpConfig" :title="mcpDialogTitle" width="600px">
      ...
      <el-tabs v-model="mcpTab" class="mcp-tabs">
        ...
        <el-tab-pane label="HTTP (Inspector 桥接)" name="http" v-if="isSingleMcp">
          <div class="mcp-code-wrap">
...
const mcpDialogDesc = ref('将以下配置添加到支持 MCP 的 AI 平台中，即可无缝接入。')
