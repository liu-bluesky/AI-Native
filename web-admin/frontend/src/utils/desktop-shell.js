...
const DESKTOP_APP_ICON_THEMES = {
  ...
  "settings-ftp-credentials": ["#38bdf8", "#0369a1"],
...
const DESKTOP_APP_ITEMS = [
  ...
  createApp({
    ...
    shortLabel: "AS",
    path: "/ai/supervision",
    summary: "按回答 ID 查看模型、工具、权限和最终回答的本地执行链路。",
  ...
  createApp({
    ...
    shortLabel: "TS",
    path: "/tasks",
    summary: "查看和恢复本机 AI Runtime 长任务，跨项目持续执行。",
  ...
  createApp({
    ...
    shortLabel: "WL",
    path: "/work-logs",
    summary: "维护项目工作日志模板、生成记录和审核状态。",
    eyebrow: "Project Work Log",
    width: 1180,
    height: 780,
    dock: false,
    launcher: false,
  ...
  createApp({
    ...
    category: "settings",
    categoryLabel: "设置应用",
    match: (path) => String(path || "").startsWith("/system/ftp-credentials"),
  }),
  createApp({
    id: "settings-email",
    label: "邮箱",
    ...
    category: "settings",
    categoryLabel: "设置应用",
    match: (path) => String(path || "").startsWith("/system/email"),
  }),
  createApp({
    id: "settings-system",
    label: "系统配置",
  ...
  createApp({
    ...
    category: "settings",
    categoryLabel: "设置应用",
    match: (path) => String(path || "").startsWith("/updates"),
  }),
  createApp({
