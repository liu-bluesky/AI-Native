...
function buildFallbackCliPrompt() {
  const sections = [
    ...
    '3. 不要依赖 description、项目说明或“当前项目”文字做绑定；需要项目绑定或续接任务树时，显式调用 `bind_project_context(...)`。',
    '4. 需要项目或规则上下文时，先读取 `get_manual_content(project_id=...)`，再按需继续查询规则、成员和技能。',
    '5. 实现型需求必须先走 `analyze_task -> resolve_relevant_context -> generate_execution_plan`，再进入执行与验证。',
    '6. 长任务先调用 `start_work_session` 获取 `session_id`，后续复用同一个 `chat_session_id/session_id`，并用 `save_work_facts`、`append_session_event` 维护轨迹。',
    '7. 如宿主支持任务树，执行前先读取 `get_current_task_tree`；完成节点时必须补验证结果后再结束。',
    '',
    '当前接入上下文：',
  ]
  ...
  if (hasProject.value) {
    sections.push(
      ...
      `- 涉及当前项目时，首轮优先使用 \`search_ids(keyword="<用户原始问题>", project_id="${normalizedProjectId.value}")\`。`,
      `- 若要创建或续接当前项目任务树，优先显式调用 \`bind_project_context(project_id="${normalizedProjectId.value}", chat_session_id="<聊天会话ID>", root_goal="<用户原始问题>")\`。`,
    )
  } else {
    sections.push('- 当前未预设默认项目；若任务明显属于某个项目，先用 `search_ids` 定位项目 ID，再调用 `bind_project_context(...)`。')
  }
