...
PUSH_PROJECT_DEPLOY_ARTIFACT_TOOL_NAME = "push_project_deploy_artifact"
DEPLOY_PROJECT_DEPLOY_ARTIFACT_TOOL_NAME = "deploy_project_deploy_artifact"
...
def project_deploy_artifact_tool_descriptors(employee_id: str = "") -> list[dict[str, Any]]:
    employee_id_value = str(employee_id or "").strip()
    return [
        {
            "tool_name": LIST_PROJECT_DEPLOY_ARTIFACTS_TOOL_NAME,
            "employee_id": employee_id_value,
            "base_tool_name": LIST_PROJECT_DEPLOY_ARTIFACTS_TOOL_NAME,
            "scoped_tool_name": LIST_PROJECT_DEPLOY_ARTIFACTS_TOOL_NAME,
            "skill_id": "__builtin__",
            "entry_name": LIST_PROJECT_DEPLOY_ARTIFACTS_TOOL_NAME,
            "script_type": "builtin",
            "description": "列出当前项目服务端部署产物记录及最近部署状态。",
            "builtin": True,
            "parameters_schema": {
                "type": "object",
                "properties": {
...
def invoke_project_builtin_tool(
    ...
    *,
    ...
    if normalized_tool_name == LIST_PROJECT_DEPLOY_ARTIFACTS_TOOL_NAME:
        payload, err = parse_object_args(args=args, args_json=args_json)
        if payload is None:
            return {"error": err}
        ...
        limit_value = max(1, min(int(payload.get("limit", 50) or 50), 100))
        artifacts = [
            _serialize_project_deploy_artifact_with_deployment(item)
            for item in project_deploy_store.list_artifacts(project_id, limit=limit_value)
        ]
        return {
            "tool_name": LIST_PROJECT_DEPLOY_ARTIFACTS_TOOL_NAME,
            "employee_id": employee_id_value,
