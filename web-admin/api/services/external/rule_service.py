"""External rule suggestion helpers."""
...
from services.catalogs.external_rule_catalog import EXTERNAL_RULE_CATALOG
...
def suggest_external_rules(
    *,
    ...
    for item in EXTERNAL_RULE_CATALOG:
        ...
        if normalized_industry and item_industries and normalized_industry not in item_industries:
            continue
        ...
        title = _normalize_match_key(item.get("title"))
        domain = _normalize_match_key(item.get("domain"))
        for token in normalized_tokens:
            if token in matched_keywords or len(token) < 3:
                continue
            if token == title or token == domain:
                score += 3
