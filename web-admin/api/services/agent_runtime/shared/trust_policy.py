"""Shared workspace trust policy for agent runtimes."""
...
class TrustPolicy:
    ...
    def get(self, workspace_path: str) -> WorkspaceTrust:
        ...
        try:
            ...
            return WorkspaceTrust(workspace_path=str(workspace_path or "").strip())
        return (
            WorkspaceTrust.from_dict(payload)
            if isinstance(payload, dict)
            else WorkspaceTrust(workspace_path=workspace_path)
        )
    ...
    def mark_trusted(
        ...
        *,
        ...
        path.write_text(
            json.dumps(trust.to_dict(), ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
        return trust
