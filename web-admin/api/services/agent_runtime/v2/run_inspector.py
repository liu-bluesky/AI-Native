...
from services.agent_runtime.core.background_process import BackgroundProcessRegistry
...
class AgentRuntimeInspector:
    def __init__(
        ...
        *,
        ...
        self._background_registry = background_registry or BackgroundProcessRegistry()
        self._checkpoint_builder = ContextCheckpointBuilder(self._transcript_store)
    ...
    def get_run_snapshot(self, run_id: str) -> dict[str, Any] | None:
        ...
        return {
            ...
            "checkpoint": self._checkpoint_builder.build(run.run_id).to_dict(),
            "background_processes": [
                handle.to_dict()
                for handle in self._background_registry.list(include_terminal=True)
                if handle.metadata.get("run_id") == run.run_id
            ],
        }
