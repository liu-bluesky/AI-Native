...
def _humanize_work_token(value: object) -> str:
    token = _normalize_text(value, 120)
    labels = {
        "analysis": "分析",
        "implementation": "实现",
        "implement": "实现",
...
def _build_executive_work_item(item: dict) -> dict:
    phases = item.get("phases") or []
    steps = item.get("steps") or []
    verification = item.get("verification") or []
    latest_status = _normalize_text(item.get("latest_status", ""), 40)
    latest_phase = _humanize_work_token(phases[0] if phases else "")
    latest_step = _humanize_work_token(steps[0] if steps else "")
    verification_summary = _normalize_text(verification[0] if verification else "", 240)
    if latest_phase and latest_step:
        work_summary = f"{latest_phase} · {latest_step}"
    else:
        work_summary = latest_step or latest_phase or "工作内容待补齐"
    return {
        "session_id": _normalize_text(item.get("session_id", ""), 80),
        "project_id": _normalize_text(item.get("project_id", ""), 80),
        "project_name": _resolve_project_name(item.get("project_id", ""), item.get("project_name", "")),
        ...
        "verification_summary": verification_summary or "还没有写入验证说明",
    }


def _build_executive_project_overview(
    *,
    days: int,
    usage: dict,
    work_sessions: dict,
    ...
    """Build a project-first view for managers without changing raw observability data."""
    projects: dict[str, dict] = {}

    def ensure_project(project_id: object, project_name: object = "") -> dict:
        normalized_project_id = _normalize_text(project_id, 80)
        normalized_project_name = _resolve_project_name(normalized_project_id, project_name)
        key = normalized_project_id or normalized_project_name or "unknown-project"
        return projects.setdefault(
            key,
            {
                "project_id": normalized_project_id,
                "project_name": normalized_project_name or normalized_project_id or "未标记项目",
                "usage_events": 0,
                "tool_calls": 0,
                "developer_count": 0,
                "active_entries": 0,
    ...
    for row in usage.get("top_projects") or []:
        project = ensure_project(row.get("project_id", ""), row.get("project_name", ""))
        project["usage_events"] += _safe_int(row.get("cnt"))
        project["tool_calls"] += _safe_int(row.get("tool_calls"))
        project["developer_count"] = max(project["developer_count"], _safe_int(row.get("developer_count")))
        project["latest_seen_at"] = _coerce_latest_token(project.get("latest_seen_at"), row.get("last_seen_at"))

    for row in work_sessions.get("top_projects") or []:
        project = ensure_project(row.get("project_id", ""), row.get("project_name", ""))
        project["session_count"] += _safe_int(row.get("session_count"))
        project["event_count"] += _safe_int(row.get("event_count"))
        project["completed_sessions"] += _safe_int(row.get("completed_sessions"))
        project["in_progress_sessions"] += _safe_int(row.get("in_progress_sessions"))
        project["blocked_sessions"] += _safe_int(row.get("blocked_sessions"))
        project["completion_rate"] = max(project["completion_rate"], _safe_float(row.get("completion_rate")))
        project["closure_rate"] = max(project["closure_rate"], _safe_float(row.get("completion_rate")))
        project["blocked_rate"] = max(project["blocked_rate"], _safe_float(row.get("blocked_rate")))
        project["latest_updated_at"] = _coerce_latest_token(project.get("latest_updated_at"), row.get("latest_updated_at"))

    for row in live_activity.get("top_projects") or []:
        project = ensure_project(row.get("project_id", ""), row.get("project_name", ""))
        project["active_entries"] += _safe_int(row.get("active_entries"))
        project["developer_count"] = max(project["developer_count"], _safe_int(row.get("developer_count")))
        project["latest_seen_at"] = _coerce_latest_token(project.get("latest_seen_at"), row.get("latest_seen_at"))

    latest_sessions = []
    for item in (work_sessions.get("recent") or [])[:10]:
        work_item = _build_executive_work_item(item)
        latest_sessions.append(work_item)
        project = ensure_project(work_item.get("project_id", ""), work_item.get("project_name", ""))
        project["work_items"].append(work_item)
        project["latest_updated_at"] = _coerce_latest_token(project.get("latest_updated_at"), work_item.get("latest_updated_at"))

    portfolio = []
    for row in projects.values():
        session_count = _safe_int(row.get("session_count"))
        completed_sessions = _safe_int(row.get("completed_sessions"))
        blocked_sessions = _safe_int(row.get("blocked_sessions"))
        usage_events = _safe_int(row.get("usage_events"))
        activity_score = (
            usage_events
            + _safe_int(row.get("tool_calls"))
            + _safe_int(row.get("event_count"))
            + _safe_int(row.get("active_entries"))
            + session_count
        )
        completion_rate = _safe_float(row.get("completion_rate"))
        if completion_rate <= 0 and session_count > 0:
            completion_rate = _percentage_value(completed_sessions, session_count)
        closure_rate = _safe_float(row.get("closure_rate")) or completion_rate
        blocked_rate = _safe_float(row.get("blocked_rate"))
        if blocked_rate <= 0 and session_count > 0:
            blocked_rate = _percentage_value(blocked_sessions, session_count)
        latest_activity_at = _coerce_latest_token(row.get("latest_updated_at"), row.get("latest_seen_at"))
        if blocked_sessions > 0:
            status = "blocked"
            status_label = "有阻塞"
            priority = 4
        elif session_count > 0 and completion_rate < 40:
            status = "at_risk"
            status_label = "闭环偏低"
            priority = 3
        elif _safe_int(row.get("in_progress_sessions")) > completed_sessions:
            status = "in_progress"
            status_label = "进行中"
            priority = 2
        ...
        portfolio.append(
            {
                **row,
                ...
                "work_items": (row.get("work_items") or [])[:3],
            }
        )

    portfolio.sort(
        key=lambda row: (
            _safe_int(row.get("risk_priority")),
            _safe_int(row.get("activity_score")),
            _normalize_text(row.get("latest_activity_at", ""), 40),
            _normalize_text(row.get("project_name", ""), 160),
        ),
        reverse=True,
    )
    risk_projects = [
        ...
        if row.get("status") in {"blocked", "at_risk"} or _safe_int(row.get("blocked_sessions")) > 0
    ][:6]
    active_projects = [row for row in portfolio if _safe_int(row.get("activity_score")) > 0]
    total_sessions = _safe_int((work_sessions.get("summary") or {}).get("total_sessions"))
    completed_sessions = _safe_int((work_sessions.get("summary") or {}).get("completed_sessions"))
    in_progress_sessions = _safe_int((work_sessions.get("summary") or {}).get("in_progress_sessions"))
    blocked_sessions = _safe_int((work_sessions.get("summary") or {}).get("blocked_sessions"))
    completion_rate = _safe_float((work_sessions.get("summary") or {}).get("completion_rate"))

    if risk_projects:
        headline = f"{len(risk_projects)} 个项目需要关注"
        recommendation = "先看阻塞工作和未闭环工作，再确认这些工作是否已经有验证结果或下一步。"
    elif active_projects:
        headline = "当前窗口的工作内容已可复盘"
        recommendation = "按项目查看这段时间做过的工作、状态和验证结果，不把闭环率误当成项目总进度。"
    else:
        headline = "当前窗口缺少工作内容数据"
        recommendation = "先补项目归因、阶段步骤和验证写回，老板视角才能复盘这段时间做了什么。"

    return {
        "version": "statistics-executive/v2",
        "window_days": days,
        "scope": scope,
        "headline": headline,
        ...
        "summary": {
            ...
            "risk_project_count": len(risk_projects),
            "total_sessions": total_sessions,
            "completed_sessions": completed_sessions,
            "in_progress_sessions": in_progress_sessions,
            "blocked_sessions": blocked_sessions,
            "completion_rate": completion_rate,
            "closure_rate": completion_rate,
        },
        "project_portfolio": portfolio[:10],
        "risk_projects": risk_projects,
        "recent_sessions": latest_sessions[:6],
    }
...
async def statistics_overview(
    ...
    executive = _build_executive_project_overview(
        days=normalized_days,
        usage=usage,
        work_sessions=work_sessions,
        live_activity=live_overview,
