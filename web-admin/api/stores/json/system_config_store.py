...
_ROOT_CAUSE_RULE_BOOTSTRAP_LINE = '19. 禁止以兜底、兼容、静默降级或重复写入多份状态来掩盖问题；遇到异常、缺失、路径不一致、状态不一致或接口不匹配时，优先定位并修正根因，收敛到唯一规范入口和 canonical 状态。只有明确处理历史数据迁移或只读恢复时，才允许短期兼容，并必须标注范围、退出条件和后续清理方案。'
_ROOT_CAUSE_RULE_GUIDE_LINE = '12. 禁止以兜底、兼容、静默降级或重复写入多份状态来掩盖问题；遇到异常、缺失、路径不一致、状态不一致或接口不匹配时，优先定位并修正根因，收敛到唯一规范入口和 canonical 状态。只有明确处理历史数据迁移或只读恢复时，才允许短期兼容，并必须标注范围、退出条件和后续清理方案。'
...
def normalize_query_mcp_usage_guide_template(template: str) -> str:
    ...
    upgraded = normalized.replace(_LEGACY_SEARCH_IDS_LINE_GUIDE, _UPDATED_SEARCH_IDS_LINE_GUIDE).replace(
