...
5. 需要在用户机器上运行打包命令时，只能通过项目聊天的命令执行能力处理，且必须已选择外部智能体并运行在桌面端 Runner；未选择外部智能体、未运行桌面端或当前电脑不可达时，停止并提示无法执行本地命令。执行本地打包命令前必须明确命令内容、工作目录、影响范围、产物路径和可恢复性，并取得用户明确授权。
...
8. 禁止扫描、读取或复用历史发布配置、CI 配置、本地凭据、远端脚本或环境变量作为执行依据；禁止把 FTP/SSH 账号密码交给外部智能体；凭据只通过项目部署配置由服务端使用，AI 侧只读到脱敏摘要。缺少桌面端 Runner、打包命令、部署产物上传能力、服务端 artifact、项目部署配置或部署产物自动部署能力时，直接报告 `blocked` / `missing`。
...
_DEPLOY_ARTIFACT_SKILL_GUIDE_LINES = [
    ...
    '1.4.4 禁止扫描、读取或复用历史发布配置、CI 配置、本地凭据、远端脚本或环境变量作为执行依据；禁止把 FTP/SSH 账号密码交给外部智能体；缺少桌面端 Runner、打包命令、部署产物上传能力、服务端 artifact、项目部署配置或部署产物自动部署能力时，直接报告 blocked / missing。',
...
_CONTINUOUS_EXECUTION_TEXT = '一旦用户已确认计划或已明确要求执行，后续按已生成计划连续推进到完成；阶段之间只更新任务树、工作事实、验证结果和必要进度，不再停下来请求“是否继续”。只有遇到破坏性/不可逆操作、权限或环境阻塞、需求范围变化、验证无法推进，或必须由用户做业务决策时，才暂停并明确说明阻塞点。'
...
_UPDATED_QUERY_MCP_ACTIVE_STATE_RECOVERY_LINE = '- 建议把客户端自生成的 chat_session_id 和 start_work_session 返回的 session_id 一起持久化；如能解析项目工作区，优先通过统一状态服务写入 `.ai-employee/query-mcp/active-sessions/<chat_session_id>.json`（窗口级/进程级权威状态）、`.ai-employee/query-mcp/session-history/<project_id>__<chat_session_id>.json`，并同步维护 `.ai-employee/requirements/<project_id>/<chat_session_id>.json`，否则再退回 CLI 自己的本地存储。这样 CLI 中断后可以直接恢复同一条任务树和工作轨迹。'
...
_CLIENT_DEPLOY_ARTIFACT_PROFILE_LINES = [
    ...
    "- 部署约束: 禁止扫描、读取或复用历史发布配置、CI 配置、本地凭据、远端脚本或环境变量作为执行依据；禁止把 FTP/SSH 账号密码交给外部智能体；凭据只通过项目部署配置由服务端使用，AI 侧只读到脱敏摘要；缺少桌面端 Runner、打包命令、部署产物上传能力、服务端 artifact、项目部署配置或部署产物自动部署能力时直接报告 `blocked` / `missing`。",
]
...
def normalize_query_mcp_bootstrap_prompt_template(template: str) -> str:
    ...
    if (
        ...
        long_task_line = "15. 长任务先调用 `start_work_session` 获取 `session_id`，后续复用同一个 `chat_session_id/session_id`，并用 `save_work_facts`、`append_session_event` 维护轨迹。"
