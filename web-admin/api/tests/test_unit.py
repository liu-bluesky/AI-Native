...
def test_project_detail_ai_deploy_routes_are_removed_but_chat_deploy_routes_remain():
    from core.server import create_app

    route_paths = {getattr(route, "path", "") for route in create_app().routes}

    assert "/api/projects/{project_id}/deploy-command/generate" not in route_paths
...
def test_project_voice_routes_remain_without_short_film_studio_routes():
    from core.server import create_app

    route_paths = {route.path for route in create_app().routes}

    assert "/api/projects/{project_id}/studio/voices" in route_paths
    assert "/api/projects/{project_id}/studio/voices/{voice_id}" in route_paths
    assert "/api/projects/{project_id}/studio/voiceovers/generate" in route_paths
    assert "/api/projects/{project_id}/studio/model-sources" in route_paths
    assert "/api/projects/{project_id}/studio/extractions" not in route_paths
    assert "/api/projects/{project_id}/studio/storyboards/generate" not in route_paths
    assert "/api/projects/{project_id}/studio/character-references/generate" not in route_paths
    assert "/api/projects/{project_id}/studio/exports" not in route_paths
    assert "/api/projects/{project_id}/studio/drafts" not in route_paths
    assert "/api/projects/{project_id}/materials" not in route_paths
    assert "/api/projects/{project_id}/materials/upload" not in route_paths
    assert "/api/projects/{project_id}/materials/{asset_id}" not in route_paths
