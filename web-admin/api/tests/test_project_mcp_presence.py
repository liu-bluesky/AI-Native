...
def test_query_mcp_runtime_returns_contextual_urls_and_cli_prompt(tmp_path, monkeypatch):
    ...
    assert "项目聊天部署约束" in runtime["cli_prompt"]
    assert "get_project_deploy_options" in runtime["cli_prompt"]
    assert "把产物推送到服务端项目详情的部署产物模块" in runtime["cli_prompt"]
    assert "自动部署由部署产物 AI/服务端部署能力" in runtime["cli_prompt"]
    assert "客户端先通过 MCP/部署技能读取项目级部署参数" not in runtime["cli_prompt"]
    assert "机器人通知群优先使用机器人连接器扫描结果" in runtime["cli_prompt"]
