...
```text
  我是按项目手册 proj-d16591a6 直接主责完成的，参考了项目里的 UI 规则 rule-134beefd。实际覆盖的工作分成三块：
  1. 后端工程师职责
     补用户/角色数据字段、排序逻辑、用户编辑接口、权限点和校验逻辑。
  2. 前端工程师职责
     重做设置中心几个列表页的筛选、分页、创建人、时间格式和统一 UI 风格。
  3. 问题修复职责
     修复 AI 生成视频素材缺封面，补自动抽帧回写和前端兜底展示。
```
