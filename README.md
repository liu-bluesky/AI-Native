...
核心位置：`web-admin/api/services/assistant_workflow_state_service.py`、`web-admin/api/services/assistant_workflow_policy_service.py`。
...
核心位置：`web-admin/api/services/assistant_capability_router_service.py`、`web-admin/api/services/runtime/tool_registry.py`。
...
核心位置：`web-admin/api/services/dynamic_mcp_apps_query.py`、`.ai-employee/skills/query-mcp-workflow/`。
...
核心位置：`web-admin/api/services/agent_runtime_v2/`。

这是后台真实执行型任务的运行时外壳。它创建 `TaskRun`，记录运行事件和对话转录，构建可恢复上下文，并根据 `assistant_workflow` 选择 `query_engine` 或 legacy orchestrator。主要子模块包括：

- `runtime.py`：组织一次 Agent 运行，建立运行上下文并流式输出结果。
- `task_run.py` / `state_store.py`：保存任务运行对象和状态。
- `query_engine.py`：驱动模型、工具调用和多轮观察。
- `dynamic_tool_pool.py` / `plugin_registry.py`：整理运行时可用工具和插件。
- `tool_execution_runner.py` / `tool_result_normalizer.py`：执行工具并标准化工具观察结果。
- `event_log.py` / `transcript_store.py` / `run_inspector.py`：记录事件、转录和运行快照，便于审计与排查。

### 7. 权限、信任与验证

核心位置：`permission_policy.py`、`permission_store.py`、`trust_policy.py`、`verification_policy.py`、`completion_policy.py`。

该模块负责判断工具调用是否需要用户授权、当前工作区是否可信、开发型任务是否必须提供验证证据，以及一次任务能否完成。完成策略会检查模型回复、后台操作、用户待办、工具错误、验证状态和任务树验证结果，避免在缺少证据时提前收口。

### 8. 中断恢复与后台操作续跑

核心位置：`resume_service.py`、`operation_resume.py`、`query_mcp_project_state.py`。

该模块负责在权限等待、后台操作、CLI 中断或页面刷新后恢复同一个任务。统一查询 MCP 会把 `chat_session_id`、`session_id`、requirement、任务树和本地 query-mcp 状态写到 `.ai-employee/` 下的 canonical 路径；Agent Runtime v2 则通过运行快照、事件日志和恢复上下文继续原来的工具调用或后台操作。
