...
- 后续开发默认围绕 `统一查询 MCP` 与 `项目 MCP` 两条主入口展开，优先补能力暴露、追踪链路、权限边界和可观测数据。
- `web-admin` 内的 AI 对话框不再作为新能力的首发开发面；它可以继续承担验证、调试和人工介入，但不应成为“能力已完成”的判断标准。
- 若某项能力需要读写代码、项目上下文、任务树、工作轨迹或统计数据，应先落到 MCP 入口、项目级 API 或可复用存储层，再决定是否补对话页按钮。
- 新增后台菜单应优先承担“观测和管理”职责，而不是把能力本身锁死在聊天界面里。

### 统计菜单定位

- 平台新增统一的 `统计` 菜单，用于集中展示 MCP 交互、工作会话、在线 MCP 活动和运行指标。
- 统计页服务于“通过 MCP 访问智能体、开发和维护项目”这条主链，优先回答哪些入口在被接入、哪些工具在被调用、哪些工作会话在推进。
- 统计页的核心作用是提供面向开发和运维的可观测面，不替代 MCP 调用，也不承担聊天式操作入口。
