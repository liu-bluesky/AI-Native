...
- 把 `AgentRuntimeEvent[]` 渲染为 NDJSON 时保持事件对象原样，每行只包含一个完整 JSON 对象。
...
Desktop/Tauri Adapter 还负责把 runtime event 通过原生事件通道广播给前端。当前事件名为 `liuagent://runtime-event`，payload 是 Core 生成的 `AgentRuntimeEvent`，不得在桥接层改写为 UI 私有结构。
