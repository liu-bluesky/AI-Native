...
权限请求不是意图识别结果。它只能绑定已经存在的 `ToolCall` 或明确的 `AdapterCommand`；禁止根据用户消息里的关键词、正则匹配或自然语言推断来提前创建删除、写入、命令执行等授权请求。没有合法结构化 `ToolCall` 时，即使用户文本包含“删除”“执行”“授权”等词，也不能弹出对应工具授权，更不能执行工具。
...
Desktop / CLI 本地 runtime 可以把 `approve_session -> session` 决策缓存到当前工作区，但缓存边界必须收敛到当前会话：

- 缓存路径：`.ai-employee/agent-runtime-v2/permissions/<chat_session_id>.json`。
- 缓存粒度：至少包含 `chat_session_id`、`action`、`source_request_id`、`grant_scope=session`、创建时间和最近使用时间。
- 缓存只允许复用同一 `chat_session_id` 下相同 `action` 的授权；不得跨项目、跨 workspace 或跨会话复用。
- 首次写入缓存必须由合法 `PermissionRequest.request_id` 对应的 `approve_session` 决策触发。
- 无 `request_id` 的外部 `approve_session` 不能直接绕过 Permission Gate；runtime 内部复用缓存时应使用不可由普通 Adapter 构造的内部标记。
- 用户拒绝、会话结束、显式清理授权或 workspace 变更时，应停止复用对应缓存。
